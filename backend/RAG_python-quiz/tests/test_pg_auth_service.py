...
class PgAuthServiceTests(unittest.TestCase):
    def test_login_success_updates_last_login_and_returns_token(self):
        ...
        self.assertIn("app_security.auth_lookup_user", cursor.executed[0][0])
    ...
    def test_logout_logs_out_valid_session_token(self):
        with patch("app.services.pg.pg_auth_service.verify_token", return_value={"sub": "user-1"}):
            result = pg_auth_service.logout("token")
    ...
    def test_register_creates_teacher_and_commits(self):
        ...
        self.assertTrue(conn.committed)
        self.assertIn("app_security.auth_email_exists", cursor.executed[0][0])
        self.assertIn("app_security.auth_register_user", cursor.executed[1][0])
