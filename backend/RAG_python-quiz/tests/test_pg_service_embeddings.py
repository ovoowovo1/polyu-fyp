...
class PgServiceEmbeddingColumnTests(unittest.TestCase):
    ...
    def test_create_graph_from_document_uses_active_embedding_column(self):
        ...
        self.assertIn("embedding", captured["sql"])
        self.assertEqual(captured["template"], "(%s,%s,%s,%s,%s,%s::vector)")
        self.assertTrue(conn.committed)

    def test_retrieve_graph_context_uses_embedding_and_filters_nulls(self):
        cursor = FakeCursor(
            fetchall_results=[
                [
                    {
    ...
    def test_retrieve_graph_context_can_use_primary_embedding_column(self):
    ...
    def test_retrieve_context_by_chunk_ids_handles_empty_cache(self):
        self.assertEqual(
            pg_retrieval_vectors.retrieve_context_by_chunk_ids(
                ...
                cached_rows=[], selected_file_ids=["file-1"],
            ),
            [],
        )
