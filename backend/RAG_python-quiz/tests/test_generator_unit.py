...
import app.agents.nodes.generator_payload as payload_module
import app.agents.nodes.generator_schema as schema_module
...
class GeneratorUnitTests(unittest.IsolatedAsyncioTestCase):
    def test_schema_prompt_and_json_helpers(self):
        mc_schema = schema_module._build_question_item_schema(
            "multiple_choice",
            bloom_enum=["remember", "apply"],
        )
        essay_schema = schema_module._build_question_item_schema(
            "essay",
            bloom_enum=["analyze"],
            answer_description="Reference answer",
        )
        section_schema = schema_module._build_generator_section_schema("essay", 2)
        ...
        prompt = schema_module._build_section_generator_prompt(
            context="context",
            difficulty="unknown",
            topic="Topic",
        ...
        self.assertEqual(
            payload_module._extract_outermost_json_object('noise {"a":"{brace}","b":1} tail'),
            '{"a":"{brace}","b":1}',
        )
        self.assertEqual(
            payload_module._extract_outermost_json_object('noise {"a":"quote \\\\\\" inner","b":1} tail'),
            '{"a":"quote \\\\\\" inner","b":1}',
        )
        self.assertIsNone(payload_module._extract_outermost_json_object("no braces here"))
        self.assertIn("char_window", payload_module._build_json_error_context("abcdef", 3, window=2))

        err = json.JSONDecodeError("bad", "{}", 1)
        self.assertIn("invalid JSON payload", payload_module._format_json_error_message(err))

    def test_payload_parsing_and_normalization_errors(self):
        with self.assertRaises(payload_module.GeneratorPayloadError):
            payload_module._parse_generator_payload("")

        payload = payload_module._parse_generator_payload('before {"multiple_choice_questions": []} after')
        self.assertEqual(payload["multiple_choice_questions"], [])

        with self.assertRaises(payload_module.GeneratorPayloadError):
            payload_module._parse_generator_payload('before {"multiple_choice_questions": [} after')

        with self.assertRaises(payload_module.GeneratorPayloadError):
            payload_module._parse_generator_payload("[]")

        normalized = payload_module._normalize_question_item(
            {
        ...
        for item, question_type in invalid_cases:
            with self.subTest(item=item):
                with self.assertRaises(payload_module.GeneratorPayloadError):
                    payload_module._normalize_question_item(
                        item,
        ...
        with self.assertRaises(payload_module.GeneratorPayloadError):
            payload_module._validate_marking_criteria([], array_name="essay_questions", question_index=0)

        normalized_section = payload_module._normalize_generator_section_payload(
            {
                "questions": [
                    {
                        "question_type": "essay",
        ...
        for payload in ({}, {"essay_questions": "bad"}, {"essay_questions": []}):
            with self.subTest(payload=payload):
                with self.assertRaises(payload_module.GeneratorPayloadError):
                    payload_module._normalize_generator_section_payload(
                        payload,
                        question_type="essay",
                        expected_count=1,
                    )
    ...
    async def test_request_section_output_handles_strict_fallback_and_failures(self):
        ...
        for finish_reason, expected in (("content_filter", "blocked"), ("length", "empty content")):
            with self.subTest(finish_reason=finish_reason):
                response = make_completion_response(finish_reason=finish_reason)
                with patch(
                    "app.agents.nodes.generator._create_section_response",
