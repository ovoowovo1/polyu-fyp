...
from app.services.rag import adaptive_retrieval_service, citation_evidence_service, retrieval_intent
from app.services.ai.ai_service import generate_structured_json
from app.services.rag.rag_shared import build_raw_sources, normalize_doc, safe_emit
...
def _initial_rag_state(question: str, selected_file_ids: Sequence[str]) -> AdaptiveRAGState:
...
def _make_queue_emit(events: asyncio.Queue[Dict[str, Any]]) -> EventCallback:
...
async def _flush_events(events: asyncio.Queue[Dict[str, Any]]) -> AsyncGenerator[Dict[str, Any], None]:
...
async def _run_rag_workflow(
    ...
    async for event in _flush_events(events):
        yield event
