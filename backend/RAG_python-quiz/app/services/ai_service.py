...
async def generate_answer_with_langchain(context: str, question: str, context_with_file_id: List[str], chunk_ids: List[str]) -> Dict[str, Any]:
    ...
    prompt = f"""
        ...
        You must use the English language in your response.
        請遵循以下推理規則：
    ...
    async def _generate_answer(api_key: str, schema: Dict[str, Any], prompt: str) -> Dict[str, Any]:
        """生成回答（用於重試機制）"""
        import json
        ...
        # 解析 JSON 回應
        text = extract_chat_completion_text(response, "答案生成")
        if not text:
            raise RuntimeError("模型回應為空")
